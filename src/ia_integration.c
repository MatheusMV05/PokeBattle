...
bool initializeAI(void) {
    // Já inicializado
    if (initialized) {
        printf("IA já inicializada, usando instância existente\n");
        return true;
    }

    printf("Tentando inicializar libcurl...\n");

    // Inicialização global da libcurl
    CURLcode globalInit = curl_global_init(CURL_GLOBAL_ALL);
    if (globalInit != CURLE_OK) {
        fprintf(stderr, "Erro ao inicializar libcurl globalmente: %d\n", globalInit);
        return false;
    }
    ...
    if (curl_handle == NULL) {
        ...
        curl_global_cleanup(); // Limpeza adequada
        return false;
    }
    ...
    // Desativar verificação SSL para desenvolvimento (não recomendado em produção)
    curl_easy_setopt(curl_handle, CURLOPT_SSL_VERIFYPEER, 0L);
    curl_easy_setopt(curl_handle, CURLOPT_SSL_VERIFYHOST, 0L);

    printf("Curl inicializado com sucesso!\n");
    initialized = true;
    return true;
}
...
void shutdownAI(void) {
    ...
    printf("Encerrando sistema de IA...\n");
    ...
    initialized = false;
    printf("Sistema de IA desligado\n");
}
...
bool testAIConnection(void) {
    ...
    printf("Testando conexão com a API Gemini...\n");
    ...
    if (response != NULL && strstr(response, "Erro:") != response) {
        ...
        if (response) {
            ...
            printf("Não foi recebida resposta da API.\n");
        }
        return false;
    }
}
